...
function computeConversationId(): string {
  ...
  // Prefix by host to avoid cross-site key collisions (e.g., gemini vs chatgpt)
...
export async function startExportButton(): Promise<void> {
  ...
  if (!isGemini && !isAltChat) return;

  // Try find a suitable host container for the button
  ...
  if (isGemini) {
    ...
    // Other chat apps: try to locate a header/nav container first
    const candidates = [
      'div[role="banner"]',
      'header[role="banner"]',
      'header',
    ...
    for (const sel of candidates) {
      logo = (await waitForElement(sel, 2000));
      if (logo) break;
    }
  }

  // Fallback: floating host at top-right if no container found
  if (!logo && isAltChat) {
    const floater = document.createElement('div');
    floater.className = 'gv-export-fab-host';
    floater.style.position = 'fixed';
    floater.style.top = '12px';
    floater.style.right = '12px';
    floater.style.zIndex = '2147483646';
    floater.style.pointerEvents = 'auto';
    try { document.body.appendChild(floater); } catch {}
    logo = floater;
  }
