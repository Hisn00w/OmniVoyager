...
export async function startPromptManager(): Promise<void> {
  try {
    ...
    gh.href = 'https://github.com/Hisn00w/OmniVoyager';
    gh.target = '_blank';
    gh.rel = 'noreferrer';
    ...
    function onReposition(): void {
      if (!open) return;      const pos = computeAnchoredPosition(trigger, panel);
      panel.style.left = `${pos.left}px`;
      panel.style.top = `${pos.top}px`;
    }

    function beginDrag(ev: PointerEvent): void {      dragging = true;
